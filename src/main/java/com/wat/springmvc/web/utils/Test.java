...
public class Test {
     ...
     */
    private static String acquireLockWithTimeOut(Jedis conn, String lockName, long waitTimeOut, int lockTimeOut) {
        String lockKey = "lock:" + lockName;
        String lockId = Math.random()*1000 + "";
     ...
     */
    private static boolean releaseLock(Jedis conn, String lockName, String lockId, long waiteTimeOut) {
        String lockKey = "lock:" + lockName;
    ...
    public static void test(int threads) {
        ...
        for (int i = 0; i < threads; i++) {
            executorService.execute(new Runnable() {
                public void run() {
                    final Jedis conn = JedisUtil.getSingletonJedis();
                    conn.select(0);
                    try {
                        latch.await();
                    } catch (InterruptedException e) {
